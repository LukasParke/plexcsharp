...
namespace LukeHagar.PlexAPI.SDK.Models.Components
{
    ...
    public static class DownloadQueueItemStatusExtension
    {
        public static string Value(this DownloadQueueItemStatus value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString())[0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static DownloadQueueItemStatus ToEnum(this string value)
        {
            foreach(var field in typeof(DownloadQueueItemStatus).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is DownloadQueueItemStatus)
                    {
                        return (DownloadQueueItemStatus)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum DownloadQueueItemStatus");
        }
    }
}